...
The `@eavios/nestjs-query-core` package provides a number of helper functions to transform or apply queries to a list of items.
...
```ts
import { applyFilter } from `@eavios/nestjs-query-core`;
...
```ts
import { applySort, SortDirection, SortNulls } from `@eavios/nestjs-query-core`;
...
```ts
import { applyPaging } from `@eavios/nestjs-query-core`;
const dtos: TestDTO[] = [
  {first: 'Bob', last: 'Yukon'}
  {first: 'Alice', last: 'Yukon'}
  {first: 'Sally', last: 'Yukon'}
...
```ts
import { applyQuery, SortDirection } from `@eavios/nestjs-query-core`;
...
```ts
import { transformFilter, QueryFieldMap, Filter } from `@eavios/nestjs-query-core`;
...
```ts
import { transformSort, QueryFieldMap, SortField, SortDirection } from `@eavios/nestjs-query-core`;
...
```ts
import { transformQuery, QueryFieldMap, Query, SortDirection } from `@eavios/nestjs-query-core`;
...
```ts
import { Filter, getFilterComparisons } from `@eavios/nestjs-query-core`;
...
```ts
import { Filter, getFilterOmitting } from `@eavios/nestjs-query-core`;

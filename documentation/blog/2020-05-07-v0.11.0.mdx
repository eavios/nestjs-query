...
In `v0.11.x` two new decorators were introduced from the `@eavios/nestjs-query-graphql` package. The `@Relation` and `@Connection` allow specifying relations on the DTO rather than passing them in as options to the `CRUDResolver`.
...
```ts {6}
import { FilterableField, Connection } from '@eavios/nestjs-query-graphql';
import { ObjectType, ID, GraphQLISODateTime } from '@nestjs/graphql';

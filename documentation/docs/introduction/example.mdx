...
Notice the use of `@FilterableField` this will let `@eavios/nestjs-query-graphql` know to allow filtering on the
...
```
**NOTE** For the sake of brevity, the `options` object in the Mongoose and Typegoose examples aren't defined. If you'd like to see full examples of all of the persistence services, please refer to the `./examples` directory in the [source code](https://github.com/eavios/nestjs-query/tree/master/examples).
